...
void couples_everyday_breakup_new_couples(vector <boy> &BOY, vector <girl> &GIRL,int day, int m)
{
	int i, j, g, b ;
	ofstream file;
    	file.open("february_breakup.txt",ios_base::app);
	for(i=m-1; i>=0; i--)
	{
		g = boy_commit[value[i]];
			b = value[i];
			
		if(happy[value[i]] <= day && boy_commit[b] != 0 && girl_commit[g] != 0)
		{
			file<<GIRL[g].name<<" is " << " going to breakup to "<<BOY[b].name <<endl;	
			girl_commit[g] = 0;
			boy_commit[b] = 0;
		}
	}
...
int main()
{
	...
	int i, day;
	printf("for the month of february having mark on it\n");
	
	for(i=1; i<29; i++)
	{
		day = i;
		couples_everyday_breakup_new_couples(BOY , GIRL ,day, m);
	}
